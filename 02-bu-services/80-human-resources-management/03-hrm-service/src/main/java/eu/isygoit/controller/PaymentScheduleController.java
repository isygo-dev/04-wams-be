...
@Validated
@RestController
@InjectMapperAndService(handler = HrmExceptionHandler.class, mapper = PaymentScheduleMapper.class, minMapper = PaymentScheduleMapper.class, service = PaymentScheduleService.class)
...
public class PaymentScheduleController extends MappedCrudController<Long, PaymentSchedule, PaymentScheduleDto, PaymentScheduleDto, PaymentScheduleService> {
    ...
    public ResponseEntity<List<PaymentScheduleDto>> calculatePaymentSchedule(@PathVariable(name = RestApiConstants.CONTRACT_ID) Long contractId) {
        try {
            List<PaymentSchedule> paymentSchedules = paymentScheduleService.calculatePaymentSchedule(contractId);
            if (CollectionUtils.isEmpty(paymentSchedules)) {
                return ResponseEntity.noContent().build();
            }
            return ResponseEntity.ok(mapper().listEntityToDto(paymentSchedules));
        } catch (Exception e) {
